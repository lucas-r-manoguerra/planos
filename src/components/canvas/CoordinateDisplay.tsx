...
export function CoordinateDisplay({ x, y }: CoordinateDisplayProps) {
  return (
    <div
      className="fixed bottom-4 right-4 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-mono text-gray-600 shadow-sm"
      aria-live="polite"
      aria-atomic="true"
      role="status"
    >
      <span className="text-gray-400">X:</span> {x} <span className="text-gray-400">Y:</span> {y} <span className="text-gray-400">cm</span>
    </div>
  );
}
